"""
processor.py — the core document processor.
Workflow: read from S3 -> extract -> validate -> retrieve policies (RAG)
          -> assess coverage -> summarize.
"""
import boto3
import json
import config
...
def process_document(bucket, key, model_id=config.DEFAULT_MODEL):
    ...
    coverage_prompt = prompts.get("assess_coverage",
